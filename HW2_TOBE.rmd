...
```{r 2.0}
elp <- read.csv('/Users/kanami/Desktop/ELP.csv', header = TRUE, sep = ',')
...
```{r 2.3.1}
M2 <- lm(data = elp, Mean_RT~log(SUBTLWF)+POS)
...
lmer(log(SUBTLWF) ~ Mean_RT + (1+Mean_RT|POS), data = elp)lmer(log(SUBTLWF) ~ Mean_RT + (1+Mean_R)
```

#### 2.4.2 Provide the variance for intercept argument for `POS` random effects
```{r 2.4.2}
summary(mix_M2)$varcor$POS[1]
```

#### 2.4.3 Add the regression line to the scatterplot
```{r 2.4.3}
data2$mix_pred <- predict(mix_M2)
ggplot(elp , aes(log(SUBTLWF), Mean_RT))+
  geom_point(aes(col = POS))+
  facet_wrap(~ POS)+
  geom_line(data = elp, aes(x = log(SUBTLWF), y = mix_pred), col = 'black', size = 0.7)
```
...
```{r 3.0}
d_caus <- read.csv("/Users/kanami/Desktop/dutch_causatives.csv")
summary(d_caus)
